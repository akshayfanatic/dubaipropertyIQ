...
import { updateAreaLocation } from '@/lib/db/areas/actions';

interface AreaLocationProps {
  areaId?: string;
  location?: LocationValue | null;
}

export function AreaLocation({ areaId, location }: AreaLocationProps) {
  ...
  const onSubmit = async (data: Location) => {
    if (!areaId) {
      toast.error('Cannot save location', {
        description: 'Please save the first basic info.',
      });
      return;
    }
    ...
    const result = await updateAreaLocation(areaId, data.location);
