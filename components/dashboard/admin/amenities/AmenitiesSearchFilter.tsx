...
const getValuesFromParams = (searchParams: URLSearchParams): AmenitiesSearchForm => ({
  search: searchParams.get('search') || '',
});

export function AmenitiesSearchFilter() {
  ...
  const methods = useForm<AmenitiesSearchForm>({
  ...
  const syncToUrl = useDebouncedCallback((values: AmenitiesSearchForm) => {
  ...
  useEffect(() => {
    ...
    if (valuesString !== lastSyncedRef.current) {
      syncToUrl(watchedValues as AmenitiesSearchForm);
    }
  ...
  return (
    <FormProvider {...methods}>
      <FilterFieldSet>
        {/* Search field - filters amenities by name */}
        <Controller name="search" control={control} render={({ field }) => <SearchInput value={field.value} onChange={field.onChange} placeholder="Search amenities..." className="max-w-sm" />} />
